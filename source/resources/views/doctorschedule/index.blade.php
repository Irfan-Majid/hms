        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor">Appointment Schedule</h3>
                </div>
                <div class="col-md-7 align-self-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                        <li class="breadcrumb-item"><a href="javascript:void(0)">Appointment Schedule</a></li>
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table color-table muted-table">
                        ...
                        @if($schedule)
                                @foreach($schedule as $i=>$u)
                                    <tr>
                                        <td>{{++$i}}</td>
                                        <td>{{$u->doctor?->name}}</td>
                                        <td>{{$u->weekday?->name}}</td>
                                        <td>{{$u->dutyshift?->start_time}} -- {{$u->dutyshift?->end_time}}</td>
                                        <td>
                                            <a href="{{route(Session::get('identity').'.doctorschedule.edit',$u->id)}}" class="btn waves-effect waves-light btn-rounded btn-xs btn-info">edit</a>
                                            <form action="{{route(Session::get('identity').'.doctorschedule.destroy',$u->id)}}" method="post" class="d-inline">
                                                    @csrf
                                                    @method('delete')
                ...
                <div>
                    <a href="{{route(Session::get('identity').'.doctorschedule.create')}}" class="btn btn-info btn-rounded mb-5">Add New Appointment Schedule</a>
                </div>
            </div>
        </div>
    </div>
