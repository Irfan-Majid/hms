        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor">TEst Invoice</h3>
                </div>
                <div class="col-md-7 align-self-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                        <li class="breadcrumb-item"><a href="javascript:void(0)">TEst INVoice</a></li>
...
<div class="row">
    <div class="col-lg-12">
              ...
              <tbody>
                            @if($datas)
                                @foreach($datas as $i=>$de)
                                    <tr>
                                        <td>{{$de->id}}</td>
                                        <td>{{$de->out_patient?->patient_id}}</td>
                                        <td>{{$de->created_at?->todatestring()}}</td>
                                        <td>{{$de->vat}}</td>
                                        <td>{{$de->discount}}</td>
                                        <td>{{$de->grand_total}}</td>
                                        <td>{{$de->paid}}</td>
                                        <td>
                                            @if($de->status==1)
                                                {{$de->grand_total-$de->paid}} 
                                            @else
                                             Paid                                      
                                            @endif
                                        </td>
                                        <td class="text-nowrap">
                                            <a href="{{route(Session::get('identity').'.testinvoice.show',$de->id)}}" data-toggle="tooltip" data-original-title="show"><i class="mdi mdi-eye text-inverse m-r-10"></i></a>
                                        @if($de->status==1)
                                            <a href="{{route(Session::get('identity').'.testinvoice.edit',$de->id)}}" data-toggle="tooltip" data-original-title="pay due"><i class="mdi mdi-medical-bag text-info m-r-10"></i> </a>
                                        @endif
        ...
        <div class="d-flex pull-right">
            <a href="{{route(Session::get('identity').'.testinvoice.create')}}" class="btn btn-info btn-rounded mb-5">Add New</a>
        </div>
    </div>
</div>
