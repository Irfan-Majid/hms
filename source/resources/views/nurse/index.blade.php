        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor">Nurse</h3>
                </div>
...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title pull-left">Nurse List</h4>
              ...
              <tbody>@php $status=array('Suspended','Active','Retired'); @endphp
                            @if($nur)
                                @foreach($nur as $i=>$nu)
                                    <tr>
                                        <td>{{$nu->id}}</td>
                                        <td>{{$nu->name}}</td>
                                        <td>{{$nu->specialization}}</td>
                                        <td>{{$nu->e_qualification}}</td>
                                        <td>{{$nu->designation}}</td>
                                        <td>{{$nu->contact}}</td>
                                        <td>{{$nu->email}}</td>
                                        <td>{{$nu->address}}</td>
                                        <td>{{$status[$nu->status]}}</td>
                                        <td>{{$nu->department_id }}</td>
                                        <td>{{$nu->employee_id}}</td>
                                        <td><img height="80" width="80" src='{{asset($nu->image)}}'></td>
                                        <td class="text-nowrap">
                                            <a href="{{route(Session::get('identity').'.nurse.edit',$nu->id)}}" class="btn waves-effect waves-light btn-rounded btn-xs btn-info">Edit</a>
                                            {{-- <a href="#" onclick="edit({{$de->id}})">edit </a> --}}
                                            {{-- <a href="{{route(Session::get('identity').'.nurse.destroy',$nu->id)}}"><i class="fa fa-close text-danger"></i> </a> --}}
                                            <form action="{{route(Session::get('identity').'.nurse.destroy',$nu->id)}}" method="post" class="d-inline">
                                                @csrf
                                                @method('delete')
                                                <button @if($nu->status==2) disabled @endif class="btn waves-effect waves-light btn-rounded btn-xs btn-danger" type="submit">Retire</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td colspan="7">
                                    {{$nur->links('pagination::bootstrap-4')}}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
            ...
            <div>
                <a href="{{route(Session::get('identity').'.nurse.create')}}" class="pull-right btn btn-info btn-rounded mb-5">Add New Nurse</a>
            </div>
        </div>
    </div>
</div>
