        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor">Appointment List</h3>
                </div>
...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title clearfix d-flex pull-left">Appointment List</h4>
                
                <div class="d-flex pull-right">
                    <a href="{{route(Session::get('identity').'.appointment.create')}}" class="btn btn-info btn-rounded mb-5">Add New Doctor</a>
                </div>
              ...
              <tbody>@php $status=array('Suspended','Active','Retired'); @endphp
                            @if($appoint)
                                @foreach($appoint as $i=>$de)
                                    <tr>
                                        <td>{{$de->id}}</td>
                                        <td>{{$de->out_patient?->name}}</td>
                                        <td>{{$de->doctor?->name}}</td>
                                        <td>{{$de->doctor?->department?->name}}</td>
                                        <td>{{$de->appoint_date}}</td>
                                        <td>{{$de->serial}}</td>
                                        <td>{{$de->problem}}</td>
                                        <td class="text-nowrap">
                                            <a href="{{route(Session::get('identity').'.appointment.show',$de->id)}}" data-toggle="tooltip" data-original-title="show"><i class="mdi mdi-eye text-inverse m-r-10"></i></a>
                                        @if($de->status==1)
                                            <a href="{{route(Session::get('identity').'.prescription.edit',$de->id)}}" data-toggle="tooltip" data-original-title="Prescribe"><i class="mdi mdi-medical-bag text-info m-r-10"></i> </a>
                                         @elseif($de->status==0)  
                                            <a href="{{route(Session::get('identity').'.prescription.show',$de->id)}}" data-toggle="tooltip" data-original-title="Show Prescription"> <i class="mdi mdi-file-document text-info m-r-10"></i></a>
                                         @endif
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td colspan="7">
                                    {{ $appoint->links('pagination::bootstrap-4') }}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
