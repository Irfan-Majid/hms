...
class CreateOperationschedulesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('operationschedules', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('in_patient_id')->unsigned()->nullable();
			$table->foreign('in_patient_id')->references('id')->on('out_patients')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('operationschedules');
    }
}
